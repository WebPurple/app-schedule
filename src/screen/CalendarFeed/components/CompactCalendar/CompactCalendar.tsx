...
export class CompactCalendar extends React.Component<Props> {
    private animatedValue: Animated.AnimatedValue = new Animated.Value(
        CLOSED_HEIHGT
    );
    ...
    private initializePanResponder() {
        ...
        this.panResponder = PanResponder.create({
            ...
            onPanResponderMove: (_e, gestureHandler) => {
                ...
                const valueToSet = isAlreadyOpened
                    ? OPENED_HEIHGT + gestureHandler.dy
                    : gestureHandler.dy;
                this.animatedValue.setValue(
                    Math.max(CLOSED_HEIHGT, valueToSet)
                );
            },
    ...
    private renderMissingWeeks(from: Date, amount: number) {
        return Array.from({ length: amount }, (_v, i: number) => {
            ...
            const weekDays: Date[] = this.generateCurrentWeekDays({
                selectedDate: currentDay,
            });
            const [props, isPresent] = this.selectRowComponent(
                weekDays,
            ...
            return (
                <Row {...props} key={currentDay.toString()}>
                    {this.renderWeek(
                        isPresent ? this.props.selectedDate : currentDay,
                        weekDays,
                        isPresent
                    )}
                </Row>
    ...
    selectRowComponent(weekDays: Date[], day: Date): [object, boolean] {
        ...
        const props = isPresent
            ...
            : {
                  ...
                  style: {
                      transform: [
                          { scale: this.scalingInterpolation },
                          { perspective: 1000 },
                      ],
                      opacity: this.rowHeightInterpolation,
    ...
    renderWeek(
        ...
        return weekDays.map(wd =>
            this.renderCell(wd, showToday && isSameDay(wd, date))
        );
    }
        ...
        return (
            <Cell key={wd.toString()}>
                <TouchableOpacity onPress={this.handlePress(wd)}>
                    <CellText underline={isCurrentDate}>
                        {format(wd, 'DD')}
                    </CellText>
                </TouchableOpacity>
            </Cell>
    ...
    render() {
        ...
        return (
            <Wrapper {...this.panResponder.panHandlers}>
                ...
                <CalendarBlock
                    as={Animated.View}
                    style={{ height: this.animatedValue }}
                >
